...
#define A_RECT_X			BUTTONS_RECT_X - CROSS_TOUCH_OVERLAP
#define A_RECT_Y			BUTTONS_RECT_Y - CROSS_TOUCH_OVERLAP
#define A_RECT_WIDTH		BUTTONS_TOUCH_WIDTH + CROSS_TOUCH_OVERLAP
#define A_RECT_HEIGHT		BUTTONS_RECT_HEIGHT + CROSS_TOUCH_OVERLAP * 2
...
#define B_RECT_X			BUTTONS_RECT_X - CROSS_TOUCH_OVERLAP
#define B_RECT_Y			BUTTONS_RECT_Y - CROSS_TOUCH_OVERLAP
#define B_RECT_WIDTH		BUTTONS_RECT_WIDTH + CROSS_TOUCH_OVERLAP * 2
#define B_RECT_HEIGHT		BUTTONS_TOUCH_WIDTH + CROSS_TOUCH_OVERLAP
...
namespace Emulator
{
	...
	{ 
		...
		this->pointersArray = ref new Platform::Array<PointerPoint ^>(ARRAY_SIZE);
	}
	...
	{		
		...
		if(!TouchButtonsIntersecting())
		{
			this->CreateTouchRectangleOnTheRight(&this->aRect, A_RECT_X_I, A_RECT_Y_I, A_RECT_WIDTH_I, A_RECT_HEIGHT_I, resolutionScale);
			this->CreateTouchRectangleOnTheRight(&this->yRect, Y_RECT_X_I, Y_RECT_Y_I, Y_RECT_WIDTH_I, Y_RECT_HEIGHT_I, resolutionScale);
			this->CreateTouchRectangleOnTheRight(&this->xRect, X_RECT_X_I, X_RECT_Y_I, X_RECT_WIDTH_I, X_RECT_HEIGHT_I, resolutionScale);
			...
			if(!IsTouchControllerOnTop())
			{
				...
				this->yRect.Y = this->aRect.Y = this->bRect.Y - this->bRect.Height;
				this->yRect.Height = this->aRect.Height = this->bRect.Height;

				this->aRect.X += (1.0f - controllerScale) * this->aRect.Width;
				this->aRect.Width *= controllerScale;

				this->yRect.X = this->aRect.X - 2.0f * this->aRect.Width;
				this->yRect.Width = this->aRect.Width;

				this->xRect.X = this->bRect.X = this->yRect.X + this->yRect.Width;
				this->xRect.Width = this->bRect.Width = this->yRect.Width;

				this->xRect.Y = this->aRect.Y - this->aRect.Height;
				this->xRect.Height = this->aRect.Height;
			}else
			{
				this->xRect.Height *= controllerScale;

				this->aRect.X += (1.0f - controllerScale) * this->aRect.Width;
				this->aRect.Width *= controllerScale;

				this->bRect.X = this->xRect.X = this->aRect.X - this->aRect.Width;
				this->bRect.Width = this->xRect.Width = this->aRect.Width;

				this->yRect.Y = this->aRect.Y = this->xRect.Y + this->xRect.Height;
				this->yRect.Height = this->aRect.Height = this->xRect.Height;

				this->yRect.X = this->aRect.X - 2.0f * this->aRect.Width;
				this->yRect.Width = this->aRect.Width;

				this->bRect.Y = this->xRect.Y + 2.0f * this->xRect.Height;
				this->bRect.Height = this->xRect.Height;
			}
		}else
		{
			this->CreateTouchRectangleOnTheRight(&this->aRect, A_RECT_X, A_RECT_Y, A_RECT_WIDTH, A_RECT_HEIGHT, resolutionScale);
			this->CreateTouchRectangleOnTheRight(&this->yRect, Y_RECT_X, Y_RECT_Y, Y_RECT_WIDTH, Y_RECT_HEIGHT, resolutionScale);
			this->CreateTouchRectangleOnTheRight(&this->xRect, X_RECT_X, X_RECT_Y, X_RECT_WIDTH, X_RECT_HEIGHT, resolutionScale);
			this->CreateTouchRectangleOnTheRight(&this->bRect, B_RECT_X, B_RECT_Y, B_RECT_WIDTH, B_RECT_HEIGHT, resolutionScale);

			this->aRect.X += (1.0f - controllerScale) * this->aRect.Width;
			this->aRect.Width *= controllerScale;
			if(!IsTouchControllerOnTop())
			{
				this->aRect.Y += (1.0f - controllerScale) * (this->aRect.Height);
			}
			this->aRect.Height *= controllerScale;

			this->yRect.X = this->aRect.X - 2.0f * this->aRect.Width;
			this->yRect.Width = this->aRect.Width;
			this->yRect.Y = this->aRect.Y;
			this->yRect.Height = this->aRect.Height;

			if(!IsTouchControllerOnTop())
			{
				this->bRect.X = this->yRect.X;
				this->bRect.Y += (1.0f - controllerScale) * this->bRect.Height;
				this->bRect.Height *= controllerScale;
				this->bRect.Width *= controllerScale;

				this->xRect.X = this->bRect.X;
				this->xRect.Y = this->yRect.Y;
				this->xRect.Width = this->bRect.Width;
				this->xRect.Height = this->bRect.Height;
			}else
			{
				this->xRect.X = this->yRect.X;
				this->xRect.Height *= controllerScale;
				this->xRect.Width *= controllerScale;

				this->bRect.X = this->xRect.X;
				this->bRect.Width = this->xRect.Width;
				this->bRect.Y = this->xRect.Y + 2.0f * this->xRect.Height;
				this->bRect.Height = this->xRect.Height;
			}
		}
		...
		this->yRect.Y -= yOffset;
		this->xRect.Y -= yOffset;
